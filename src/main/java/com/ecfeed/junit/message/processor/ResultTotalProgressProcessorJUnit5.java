...
public final class ResultTotalProgressProcessorJUnit5 implements ArgumentChainJUnit5 {
	
	private final ArgumentChainJUnit5 fChain;
	
	public ResultTotalProgressProcessorJUnit5(ArgumentChainJUnit5 chain) {
		fChain = chain;
	}
	...
	public Optional<Arguments> process(String json) {
		...
		try {
			Logger.message(Localization.bundle.getString("argumentChainErroneousJsonDataInfo") + " " + fMapper.reader().forType(ResultTotalProgressSchema.class).readValue(json));
